...
for i = 0, 20 do
    for j = 0, 20 do
        ...
        if i+j<=10 then say(1, 'x+y', i, j, i+j) end

        -- 2. subtraction within 10 (x-y)
        if i<=10 and i-j>=0 then say(2, 'x-y', i, j, i-j) end

        -- 3. carry-save addition within 20 (x+y)
        if (i>=10 or j>=10) and i+j<=20 then say(3, 'x+y', i, j, i+j) end

        -- 4. subtraction without abdication within 20 (x-y)
        if i>=10 and i-j>=0 and i%10>=j%10 then say(4, 'x-y', i, j, i-j) end

        -- 5. addition with carry within 20 (x+y)
        if i<10 and j<10 and i+j>10 then say(5, 'x+y', i, j, i+j) end

        -- 6. subtraction with abdication within 20 (x-y)
        if i-j>0 and i%10<j%10 then say(6, 'x-y', i, j, i-j) end

        -- 7-14 3 operands
        for k=0,20 do
            -- 7. continuous addition within 10 (x+y+z)
            if i+j+k <= 10 then say(7, 'x+y+z', i, j, k) end

            -- 8. continuous subtraction within 10 (x-y-z)
            if i<=10 and i-j-k>=0 then say(8, 'x-y-z', i, j, k) end

            -- 9. subtraction and addition within 10 (x-y+z)
            if i<=10 and i-j>=0 and i-j+k<=10 then say(9, 'x-y+z', i, j, k) end

            -- 10. addition and subtraction within 10 (x+y-z)
            if i+j<=10 and i+j-k>=0 then say(10, 'x+y-z', i, j, k) end

            -- 11. continuous addition within 20 (x+y+z)
            if i+j+k>10 and i+j+k<=20 then say(11, 'x+y+z', i, j, k) end

            -- 12. continuous subtraction within 20 (x-y-z)
            if i>10 and i-j-k>=0 then say(12, 'x-y-z', i, j, k) end

            -- 13. subtraction and addition within 20 (x-y+z)
            if i-j>=0 and i-j+k>10 and i-j+k<=20 then say(13, 'x-y+z', i, j, k) end

            -- 14. addition and subtraction within 20 (x+y-z)
            if i+j<=20 and i+j-k>=0 then say(14, 'x+y-z', i, j, k) end
        end
    end
end
...
for i=20,100 do
    for j=0,100,10 do
        ...
        if i+j<=100 then say(15, 'x+y', i, j, i+j); say(15, 'x+y', j, i, i+j) end

        -- 16. subtraction without abdication of whole 10 within 100 (x-y)
        if i-j>=0 then say(16, 'x-y', i, j, i-j) end
    end 
end
...
for i=10,100 do
    for j=0,10-1 do
        ...
        if i+j<=100 and i%10+j<10 then say(17,'x+y',i,j,i+j); say(17,'x+y',j,i,i+j) end

        -- 18. subtraction without abdication of single digit within 100 (x-y)
        if i-j>=0 and i%10>=j then say(18,'x-y',i,j,i-j) end

        -- 19. addition with carry of single digit within 100 (x+y)
        if i+j<=100 and i%10+j>=10 then say(19,'x+y',i,j,i+j) end

        -- 20. subtraction with abdication of single digit within 100 (x-y)
        if i-j>=0 and i%10<j then say(20,'x-y',i,j,i-j) end
    end
end
...
for i=10,100-1 do
    for j=10,100-1 do
        ...
        if i+j<=100 and i%10+j%10<10 then say(21,'x+y',i,j,i+j) end

        -- 22. subtraction without abdication of double-digit within 100 (x-y)
        if i-j>=0 and i%10>=j%10 then say(22,'x-y',i,j,i-j) end

        -- 23. addition with carry of double-digit within 100 (x+y)
        if i+j<=100 and i%10+j%10>=10 then say(23,'x+y',i,j,i+j) end

        -- 24. subtraction without abdication of double-digit within 100 (x-y)
        if i-j>=0 and i%10<j%10 then say(24,'x-y',i,j,i-j) end
    end 
end
...
for i=0,100-1 do
    for j=0,100-1 do
        for k=0,100-1 do
            if i+j+k<=100 and i+j+k>20 then say(25,'x+y+z',i,j,k) end
        end
    end
end
...
for i=21,100-1 do
    for j=0,100-1 do
        for k=0,100-1 do
            if i-j-k>=0 then say(26,'x-y-z',i,j,k) end
        end
    end
end
...
for i=0,10-1 do
    for j=0,10-1 do
        ...
        for k=0,100-1 do
            ...
            if i*j-k>=0 and i*j%10>=k%10 then say(35,'x*y-z',i,j,k) end

            -- 36. subtraction without abdication and multiplication (x-y*z)
            if k-i*j>=0 and k%10>=i*j%10 then say(36,'x-y*z',k,i,j) end
            ...
            if i~=0 and k-j>=0 and k%10>=j%10 then say(44,'x-y/z',k,i*j,i) end

            if i~=0 and j+k<=100 and j%10+k%10>=10 then
                -- 45. division and addition with carry (x/y+z)
                say(45,'x/y+z',i*j,i,k)

                -- 46. addition with carry and division (x+y/z)
                say(46,'x+y/z',k,i*j,i)
            end
            ...
            if i~=0 and k-j>=0 and k%10<j%10 then say(47,'x-y/z',k,i*j,i) end
        end
    end
end
...
for i=100,1000-100,100 do
    for j=100,1000-100,100 do
        ...
        if i+j<=1000 then say(48,'x+y/10',floor(i/10),floor(j/10),floor((i+j)/10)) end

        -- 49. subtraction of whole 100 (x-y)
        if i>=j then say(49,'x-y/10',floor(i/10),floor(j/10),floor((i-j)/10)) end
    end
end
...
for i=10,1000-10,10 do
    for j=10,1000-10,10 do
        ...
        if i+j<=1000 and i%100+j%100<100 then say(50,'x+y/10',floor(i/10),floor(j/10),floor((i+j)/10)) end

        -- 51. addition with carry of whole 100 and whole 10 (x+y)
        if i+j<=1000 and i%100+j%100>=100 then say(51,'x+y/10',floor(i/10),floor(j/10),floor((i+j)/10)) end

        -- 52. subtraction without abdication of whole 100 and whole 10 (x-y)
        if i>=j and i%100>=j%100 then say(52,'x-y/10',floor(i/10),floor(j/10),floor((i-j)/10)) end

        -- 53. subtraction with abdication of whole 100 and whole 10 (x-y)
        if i>=j and i%100<j%100 then say(53,'x-y/10',floor(i/10),floor(j/10),floor((i-j)/10)) end
    end
end
